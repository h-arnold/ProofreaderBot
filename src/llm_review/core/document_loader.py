...
def load_issues(
    ...
    *,
    ...
    for subject, issue in raw_issues_with_subjects:
        ...
        if (
            documents_lower is not None
            and issue.filename.lower() not in documents_lower
        ):
            continue
    ...
    with open(report_path, "r", encoding="utf-8", newline="") as f:
        ...
        for row_num, row in enumerate(reader, start=2):  # Start at 2 (1 is header)
            try:
                ...
                suggestions = (
                    [s.strip() for s in suggestions_str.split(",") if s.strip()]
                    if suggestions_str
                    else []
                )
                ...
                print(
                    f"Warning: Skipping malformed row {row_num}: {e}", file=sys.stderr
                )
                continue
...
def _validate_markdown_files(
    ...
    for key, issues in grouped.items():
        ...
        if not markdown_path.exists():
            print(
                f"Warning: Markdown file not found for {key}: {markdown_path}",
                file=sys.stderr,
            )
            print(
                f"  Skipping {len(issues)} issue(s) for this document", file=sys.stderr
            )
            continue
