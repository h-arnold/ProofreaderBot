...
- `PARSING_ERROR`: Mechanical/string issues (missing hyphen, merged words, stray spacing, words with similar shape to the misspelling e.g. `Oueen` instead of `Queen`, `vntil` instead of `until` or `ves` isntead of `yes`.).
...
When LanguageTool suggests adding a comma before a conjunction (e.g., "and", "but", "or") that separates two independent clauses, note this as `STYLISTIC_PREFERENCE` unless the absence of the comma creates ambiguity or misleads the reader.