...
def test_create_converter_invalid_type() -> None:
    ...
    try:
        ...
        def test_create_docling_converter_case_insensitive() -> None:
            ...
            conv1 = create_converter("docling")
            conv2 = create_converter("DOCLING")

            assert isinstance(conv1, DoclingConverter)
            assert isinstance(conv2, DoclingConverter)
        ...
        def test_docling_converter_convert_uses_docling(
            ...
            try:
                ...
                result = converter.convert(fake_pdf)
                assert isinstance(result, ConversionResult)
                assert result.markdown == "# Docling Converted"
                assert result.metadata == {}

                # Ensure underlying converter was called with the Path we passed
                assert isinstance(converter._converter, DummyDocumentConverter)
                assert converter._converter.convert_called_with == fake_pdf
            finally:
                converter.close()
