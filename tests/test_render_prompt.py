...
class TestReadPrompt:
    ...
    def test_read_language_tool_categoriser(self) -> None:
        ...
        content = _read_prompt("language_tool_categoriser.md")
        assert isinstance(content, str)
        assert len(content) > 0
...
class TestRenderTemplate:
    ...
    def test_render_language_tool_categoriser(self) -> None:
        ...
        result = render_template("language_tool_categoriser.md")
        assert isinstance(result, str)
        assert len(result) > 0
        # Should contain the partial content (from llm_reviewer_system_prompt)
        assert "WJEC" in result
        # Should contain the original template content
        assert "Language Tool Report" in result or "Error Categories" in result

    def test_render_with_empty_context(self) -> None:
        """Test rendering with explicit empty context."""
        result1 = render_template("language_tool_categoriser.md", {})
        result2 = render_template("language_tool_categoriser.md")
    ...
    def test_render_contains_system_prompt(self) -> None:
        ...
        result = render_template("language_tool_categoriser.md")
        # The partial should be expanded, so we check for content from both files
    ...
    def test_render_no_fences_in_partial(self) -> None:
        ...
        assert result.count("```markdown") == 0 or result.count("```markdown") < 3
...
class TestRenderPromptEdgeCases:
    ...
    def test_render_handles_special_characters(self) -> None:
        ...
        result = render_template("language_tool_categoriser.md")
        # language_tool_categoriser contains JSON examples with special chars
    ...
    def test_render_long_template(self) -> None:
        ...
        # language_tool_categoriser is a fairly long template
        result = render_template("language_tool_categoriser.md")
    ...
    def test_partial_content_integration(self) -> None:
        ...
        result = render_template("language_tool_categoriser.md")
        # Both files have the term "WJEC" and "error" (or "Error")
        assert "WJEC" in result
        # The system prompt mentions "proofreader"
        assert "proofreader" in result.lower()
