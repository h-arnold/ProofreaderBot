# WjecDocumentScraper — Agent Instructions
...
- CLI behaviour or options: Modify `src/cli/__init__.py` (keep `--subjects`, `--list-subjects`, `--dry-run`, `-o/--output`).
...
- docs/manage_language_ignore.md — guide for amending `DEFAULT_IGNORED_WORDS`. Read before editing `scripts/manage_language_ignore.py` or its payload JSON.
